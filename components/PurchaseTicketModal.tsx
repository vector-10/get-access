...
//     if (!user || !userHasWallet(user as any)) return 0;
    
//     try {
//       const connection = new Connection(process.env.NEXT_PUBLIC_SOLANA_RPC_URL || 'https://api.devnet.solana.com');
      
//       // Type assertion for Civic user with wallet
//       const walletUser = user as any;
//       const publicKey = walletUser.solana?.wallet?.publicKey;
...
//           walletAddress: (user as any).solana?.address
//         }),
//       });
...
//         {/* Wallet Info */}
//         {user && userHasWallet(user as any) && (
...
//               Address: {(user as any).solana?.address?.slice(0, 8)}...{(user as any).solana?.address?.slice(-8)}
//             </p>
...
export default function PurchaseTicketModal({ 
  ...
  // Remove the checkWalletBalance function completely - we're simulating everything
  ...
  const handlePurchase = async () => {
    ...
    // REMOVE ALL WALLET CHECKS - Just simulate the flow
    setLoading(true);
    ...
    try {
      ...
      toast.loading('🔍 Connecting to Civic wallet...', { id: 'purchase' });
      await new Promise(resolve => setTimeout(resolve, 1500));

      // Step 2: Validating balance (simulate success)
      toast.loading('💰 Validating SOL balance (5.25 SOL available)...', { id: 'purchase' });
      await new Promise(resolve => setTimeout(resolve, 1500));

      // Step 3: Processing payment
      toast.loading('⚡ Processing payment on Solana network...', { id: 'purchase' });
      await new Promise(resolve => setTimeout(resolve, 2000));

      // Step 4: Create ticket record
      toast.loading('📝 Creating ticket record...', { id: 'purchase' });
      ...
      const response = await fetch('/api/purchase-ticket', {
        ...
        body: JSON.stringify({
          ...
          price: ticketPrice,
          paymentMethod: 'embedded_wallet',
          walletAddress: `civic_${userId.slice(0, 8)}`
        }),
      });
      ...
      toast.success('✅ Payment confirmed! Minting your NFT...', { id: 'purchase' });
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      // Step 6: NFT minting simulation
      toast.loading('🎨 Minting NFT on Solana blockchain...', { id: 'purchase' });
      await new Promise(resolve => setTimeout(resolve, 2500));
      
      // Step 7: Final success
      toast.success('🎫 NFT Ticket minted and added to wallet!', { id: 'purchase' });
      setShowNFTSuccess(true);
    ...
    return (
      <div className="fixed inset-0 bg-black/60 bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full text-center animate-bounce">
          ...
          <h3 className="text-2xl font-bold text-gray-900 mb-3">🎉 NFT Ticket Minted!</h3>
          <p className="text-gray-600 mb-4">Your ticket has been successfully minted as an NFT on Solana blockchain and added to your wallet!</p>
          
          <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4 mb-6">
            <p className="text-sm text-gray-700 mb-2"><span className="font-medium">Event:</span> {event.name}</p>
            <p className="text-sm text-gray-700 mb-2"><span className="font-medium">Type:</span> <span className="capitalize">{ticketType}</span></p>
            <p className="text-sm text-gray-700 mb-2"><span className="font-medium">Price:</span> {ticketPrices[ticketType as keyof typeof ticketPrices]} SOL</p>
            <p className="text-sm text-gray-700"><span className="font-medium">NFT ID:</span> SOL_{Date.now()}</p>
          </div>

          <div className="text-center">
            <div className="flex justify-center space-x-2 mb-4">
              <span className="w-3 h-3 bg-green-500 rounded-full animate-ping"></span>
              <span className="w-3 h-3 bg-blue-500 rounded-full animate-ping" style={{animationDelay: '0.1s'}}></span>
              <span className="w-3 h-3 bg-purple-500 rounded-full animate-ping" style={{animationDelay: '0.2s'}}></span>
            </div>
            <p className="text-xs text-gray-500">✨ NFT being added to your Civic wallet...</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="fixed inset-0 bg-black/60 bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          ...
          <p className="text-sm text-blue-700">
            Address: {user?.id?.slice(0, 8)}...{user?.id?.slice(-8)}
          </p>
          <p className="text-xs text-blue-600 mt-1">
            💰 Balance: 5.25 SOL • ⚡ Ready for payment
          </p>
        </div>
